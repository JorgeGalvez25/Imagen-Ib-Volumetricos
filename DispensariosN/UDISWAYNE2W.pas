...
type
  TFDISWAYNE2W = class(TForm)
    ...
    PuertoSerial1: TApdComPort;
    Timer1: TTimer;
    DBGrid3: TDBGrid;
    CheckBox1: TCheckBox;
    PuertoSerial2: TApdComPort;
    ...
    procedure PuertoSerial1TriggerAvail(CP: TObject; Count: Word);
    procedure Timer1Timer(Sender: TObject);
    procedure PuertoSerial2TriggerAvail(CP: TObject; Count: Word);
  private
    { Private declarations }
    etTimeOut1 : EventTimer;
    ...
    SwPasoBien      :boolean;
    { Private declarations }
     function  TransmiteComandoPS1(DataBlock:string;xintentos:integer):boolean;
     function  TransmiteComandoPS2(DataBlock:string;xintentos:integer):boolean;
  public
    { Public declarations }
    ...
    //procedure PonPuertoSegmento(xseg:integer);
...
procedure TFDISWAYNE2W.PuertoSerial1TriggerAvail(CP: TObject; Count: Word);
var i : integer;
begin
   for i:=1 to Count do begin
     sRespuesta1:= sRespuesta1 + PuertoSerial1.GetChar;
   end;
   i:= length(sRespuesta1);
   if ( ( i>=iBytesEsperados ) ) then
      bListo1:= true
   else
      newtimer(etTimeOut1,MSecs2Ticks(dmcons.WtwTimeout));
end;

function TFDISWAYNE2W.TransmiteComandoPS1(DataBlock:string;xintentos:integer):boolean;
var ss:string;
    iMaxIntentos,i,
    iNoIntento    :integer;
    bOk           :boolean;
begin
   iMaxIntentos:=xintentos;
   iBytesEsperados:=13;
   iNoIntento:= 0;
   bOk:=false;
   repeat
      ...
      bListo1:= false;
      sRespuesta1:= '';
      PuertoSerial1.FlushInBuffer;
      PuertoSerial1.FlushOutBuffer;
      ss:=StrToHexSep(DataBlock);
      DMCONS.AgregaLog('E1 '+ss);
      for i:= 1 to length ( DataBlock ) do begin
         PuertoSerial1.PutChar(DataBlock[i]);
         repeat
            PuertoSerial1.ProcessCommunications;
         until ( PuertoSerial1.OutBuffUsed=0 );
      end;
      if ( not bOk ) then begin
         newtimer(etTimeOut1,MSecs2Ticks(dmcons.WtwTimeout));
         repeat
            Application.ProcessMessages;
         until ( ( bListo1 ) or ( timerexpired(etTimeOut1) ) );
         if ( bListo1 ) then begin
           if length(sRespuesta1)=13 then begin
             bOk:=true;
             DMCONS.AgregaLog('R  '+StrToHexSep(sRespuesta1));
           end;
         end;
         if ( not bOk ) then begin
...
function TFDISWAYNE2W.TransmiteComandoPS2(DataBlock:string;xintentos:integer):boolean;
var ss:string;
    iMaxIntentos,i,
    iNoIntento    :integer;
    bOk           :boolean;
begin
   ...
   repeat
      ...
      bListo2:= false;
      sRespuesta2:= '';
      PuertoSerial2.FlushInBuffer;
      PuertoSerial2.FlushOutBuffer;
      ss:=StrToHexSep(DataBlock);
      DMCONS.AgregaLog('E2 '+ss);
      for i:= 1 to length ( DataBlock ) do begin
         PuertoSerial2.PutChar(DataBlock[i]);
         repeat
            PuertoSerial2.ProcessCommunications;
         until ( PuertoSerial2.OutBuffUsed=0 );
      end;
      if ( not bOk ) then begin
         newtimer(etTimeOut2,MSecs2Ticks(dmcons.WtwTimeout));
         repeat
            Application.ProcessMessages;
         ...
         if ( bListo2 ) then begin
           if length(sRespuesta2)=13 then begin
             bOk:=true;
             DMCONS.AgregaLog('R  '+StrToHexSep(sRespuesta2));
           end;
         end;
         if ( not bOk ) then begin
            if  ( iNoIntento<iMaxIntentos ) then sleep(dmcons.GtwTiempoCmnd);
...
begin
  try
    with DMCONS do begin
      PuertoSerial1.Open:=false;
      PuertoSerial1.Baud:= ptBaudios;
      PuertoSerial1.DataBits:= ptBitsDatos;
      case ptParidad[1] of
        'N':PuertoSerial1.Parity:=pNone;
        'E':PuertoSerial1.Parity:=pEven;
        'O':PuertoSerial1.Parity:=pOdd;
      end;
      PuertoSerial1.StopBits:=ptBitsParada;
      PuertoSerial1.ComNumber:= ptPuerto;

      PuertoSerial1.TraceAllHex:= true;
      PuertoSerial1.TraceName:= 'c:\IgasTrace.txt';
      PuertoSerial1.Tracing:= tlOn;

      PuertoSerial1.Open:= true;

      // ---

      if MaxPosCarga>=WtwPosIniExt then begin
        PuertoSerial2.Open:=false;
        PuertoSerial2.Baud:= ptBaudios;
        PuertoSerial2.DataBits:= ptBitsDatos;
        case ptParidad[1] of
          'N':PuertoSerial2.Parity:=pNone;
          'E':PuertoSerial2.Parity:=pEven;
          'O':PuertoSerial2.Parity:=pOdd;
        end;
        PuertoSerial2.StopBits:=ptBitsParada;
        PuertoSerial2.ComNumber:= WtwComExt;

        PuertoSerial2.TraceAllHex:= true;
        PuertoSerial2.TraceName:= 'c:\IgasTrace2.txt';
        PuertoSerial2.Tracing:= tlOn;
        PuertoSerial2.Open:= true;
      end;
...
begin
  try
    PuertoSerial1.Tracing:= tlOff;
    PuertoSerial1.Open:= false;
    PuertoSerial1.DTR:= false;
    PuertoSerial1.RTS:= false;
    result:= true;
    if MaxPosCarga>=DMCONS.WtwPosIniExt then begin
      PuertoSerial2.Tracing:= tlOff;
      PuertoSerial2.Open:= false;
      PuertoSerial2.DTR:= false;
      PuertoSerial2.RTS:= false;
      result:= true;
    end;
   except
      result:= false;
...
begin
   ...
   if xTipoPreset=1 then begin  // Pesos
     ...
     if SegmActual=1 then begin
       if ( ( TransmiteComandoPS1(DataBlock,2) ) and ( length(sRespuesta1)=13 ) ) then
         result:=true;
     end
     else begin
       if ( ( TransmiteComandoPS2(DataBlock,2) ) and ( length(sRespuesta2)=13 ) ) then
         result:=true;
     end;
   end
...
begin
   ...
   if xpm in [0..4] then begin
     ...
     //if ( ( TransmiteComando1(DataBlock,2) ) and ( length(sRespuesta)=13 ) ) then
      // result:=true;
   end;
end;
...
begin
   ...
   if xpos in [1..4] then begin
     ...
     if SegmActual=1 then begin
       if ( ( TransmiteComandoPS1(DataBlock,2) ) and ( length(sRespuesta1)=13 ) ) then begin
         ss:=StrToHexSep(DesempacaWayne(sRespuesta1));
       ...
       if ( ( TransmiteComandoPS2(DataBlock,2) ) and ( length(sRespuesta2)=13 ) ) then begin
...
begin
   ...
   if SegmActual=1 then begin
     if ( ( TransmiteComandoPS1(DataBlock,1) ) and ( length(sRespuesta1)=13 ) ) then begin
       sRespuesta1:=DesempacaWayne(sRespuesta1);
       StrBin:=ByteToBin(ord(sRespuesta1[5]));
       for i:=0 to 7 do
         tbit[i]:=(strbin[8-i]='1');
     ...
     if ( ( TransmiteComandoPS2(DataBlock,1) ) and ( length(sRespuesta2)=13 ) ) then begin
...
var
  ...
  estado:integer;
begin
  with DMCONS,PuertoSerial1 do begin
...
begin
  ...
  PuertoSerial1.TraceName:= '\ImagenCo\Trace1'+FiltraStrNum(FechaHoraToStr(Now))+'.Txt';
  PuertoSerial1.Tracing:= tlDump;
  PuertoSerial1.Tracing:= tlOn;

  if MaxPosCarga>=DMCONS.WtwPosIniExt then begin
    PuertoSerial2.TraceName:= '\ImagenCo\Trace2'+FiltraStrNum(FechaHoraToStr(Now))+'.Txt';
    PuertoSerial2.Tracing:= tlDump;
    PuertoSerial2.Tracing:= tlOn;
  end;
end;
...
procedure TFDISWAYNE2W.PonPuertoPos(xpos:integer);
begin
  with DMCONS do begin
    if MaxPosCarga>=WtwPosIniExt then begin
...
begin
  try
    if not PuertoSerial1.Open then
      if not AbrePuerto then
        raise exception.Create('No puedo abrir puerto');
    xfec:=Now;
    with DMCONS do if checkbox1.Checked then begin
      if abs(xfec-FechaInicioAp)>60*TMMinuto then begin
        swreset:=true;
        for xpos:=1 to MaxPosCarga do
          if TPosCarga[PosCiclo].estatus>1 then
            swreset:=false;
        if swreset then begin // cierra
          SwCerrar:=true;
          FDISWAYNE2W.Close;
        end;
      end;
        ...
        case NumPaso of
          ...
          1:if (stciclo=xciclo)or(Estatus>1) then begin // ESTATUS
              try
                if not swdeshabil then begin   // no polea los que estan deshabilitados
                  ...
                  else if (estatus=1)and(estatusant<>1) then
                    EsperaFinVenta:=0;
                  EstatusDispensarios;
          ...
          2:if (swleeventa)and(estatus>0) then begin       // LEE VENTA TERMINADA
              if not swdeshabil then begin   // no polea los que estan deshabilitados
                ...
                if DameLecturas(PosCiclo,zVolumen,zPrecio,zImporte) then begin
                  importe:=zimporte;
                  volumen:=zvolumen;
                  precio:=zprecio;
                  swleeventa:=false;
...
procedure TFDISWAYNE2W.PuertoSerial2TriggerAvail(CP: TObject; Count: Word);
var i : integer;
begin
   for i:=1 to Count do begin
     sRespuesta2:= sRespuesta2 + PuertoSerial2.GetChar;
   end;
   i:= length(sRespuesta2);
   if ( ( i>=iBytesEsperados ) ) then
      bListo2:= true
   else
      newtimer(etTimeOut2,MSecs2Ticks(dmcons.WtwTimeout));end;
