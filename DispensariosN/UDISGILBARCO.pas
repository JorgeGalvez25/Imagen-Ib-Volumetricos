          ...
          2:if (swleeventa)and(estatus>0) then begin       // LEE VENTA TERMINADA
              if TPosCarga[PosCiclo].DigitosGilbarco=6 then begin
                ...
                begin
                  ...
                  if (volumen>0) or (TPosCarga[PosCiclo].IntentosFinV=0) or (not SwAvanzoVenta) then begin
                    swleeventa:=false;
                    SwAvanzoVenta:=False;
                  end
                  else
                    Dec(TPosCarga[PosCiclo].IntentosFinV);
                end;
              end
              else begin
        ...
        if (NumPaso=7) then begin
          ...
          for xpos:=1 to MaxPosCarga do
            for xp:=1 to TPosCarga[xpos].NoComb do
              if TPosCarga[xpos].TCambioPrecN1[xp] then
                swprec:=true;
          if not swprec then
            NumPaso:=8;
        end;
