...
type
  ...
  public
    ...
    WtwDivLitros,           // Divisor litros para ventas                             **
    WtwComExt,              // Puerto Com extendido
    WtwPosIniExt,           // Posicion Inicial extendida
    ...
    // WAYNE
    WtwTimeOut :integer;             // Timeout miliseg
    //WayneFusion,
    MapeoFusion,
...
uses UIMGUTIL, DECUTIL, Cipher, Hashes, IdHash, IdHashMessageDigest;
        ...
        while not Q_Prec.Eof do begin
          ...
          if xcomb in [1..MaxComb] then
            if TabComb[xcomb].Activo then begin
              ...
              TabComb[xcomb].PrecioNuevo:=Q_PrecPrecio.AsFloat;
              ...
              for xc:=1 to MaxComb do
                if TabComb[xc].Agruparcon=xcomb then begin
                  ...
                  TabComb[xc].PrecioNuevo:=Q_PrecPrecio.AsFloat;     
...
begin
  ...
  GtwTimeOut:=300;
  GtwTiempoCmnd:=100;
  if T_EstsIbTipoDispensario.AsInteger=7 then begin
    GtwTiempoCmnd:=150;
    GtwTimeOut:=2000;
    GtwKairosFrec:=1;
  end;
  ...
  try
    ...
    for i:=0 to lista.Count-1 do begin
      ...
      if Mayusculas(ss)='TRESDECIMVOLTEAM' then
        TresDecimVolTEAM:=ExtraeElemStrSep(lin,2,'=');
      if Mayusculas(ss)='DECIMPRESETTEAM' then
        DecimPresetTEAM:=ExtraeElemStrSep(lin,2,'=');
      ...
      if Mayusculas(ss)='WTWCOMEXT' then
        WtwComExt:=StrToIntDef(ExtraeElemStrSep(lin,2,'='),1);
      if Mayusculas(ss)='WTWPOSINIEXT' then
        WtwPosIniExt:=StrToIntDef(ExtraeElemStrSep(lin,2,'='),17);
      ...
      if Mayusculas(ss)='GTWKAIROSFREC' then begin
        GtwKairosFrec:=StrToIntDef(ExtraeElemStrSep(lin,2,'='),2);
        if not (GtwKairosFrec in [1..3]) then
          GtwKairosFrec:=1;
      end;
      ...
      if Mayusculas(ss)='POSICIONPRODUCTOWAYNE' then
        PosicionProductoWayne:=ExtraeElemStrSep(lin,2,'=');
