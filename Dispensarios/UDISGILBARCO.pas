...
const
      ...
      xTiempoCmnd=200;  // Miliseg      : Pausa despues de enviar un comando

      (*
      xDivPresetLts=100;    // Divisor preset litros           **
      xDivPresetPesos=100;  // Divisor preset pesos            **

      xDivPrecio=100;       // Divisor precio para lecturas y cambio de precios       **
      xDivImporte=1000;     // Divisor importe para lecturas y ventas                 **
      xDivLitros=1000;      // Divisor litros para ventas                             **

      xDivTotLts=100;       // Divisor totales litros     **
      xDivTotImporte=100;     // Divisor totales pesos      **

      xDigitos=8;           // Configuracion digitos 6 u 8
...
procedure TFDISGILBARCO.DespliegaPrecios;
var i:integer;
begin
  with DMCONS do begin
    Memo1.Lines.Clear;
  ...
  try
    ...
    with DMCONS do begin
      if swcierrabd then begin
        ...
        swcierrabd:=false;
        Q_CombIb.Active:=true;
        DespliegaMemo4('Cierro y abro DB');
      end;
                ...
                if rsp='OK' then begin
                  ...
                  if rsp='OK' then begin
                    if (TPosCarga[xpos].estatus in [1,5,9]) then begin
                      ...
                      // Preset Pesos
                      //DespliegaMemo4('E> P '+inttostr(xpos)+' '+formatfloat('#0.00',ximporte));
                      if TPosCarga[xPos].DigitosGilbarco=6 then begin
                        if EnviaPresetBomba6(xpos,xp,1,ximporte,0) then
                        begin
                          //DespliegaMemo4('R> P '+inttostr(xpos));
                ...
                if rsp='OK' then begin
                  ...
                  if rsp='OK' then begin
                    if (TPosCarga[xpos].estatus in [1,5,9]) then begin
                      ...
                      TPosCarga[xpos].Esperafinventa:=StrToIntDef(ExtraeElemStrSep(TabCmnd[xcmnd].Comando,6,' '),0);
                      // Preset Litros
                      //DespliegaMemo4('E> P '+inttostr(xpos)+' '+formatfloat('#0.00',xlitros));
                      if TPosCarga[xPos].DigitosGilbarco=6 then begin
          ...
          // CMND: DESAUTORIZA VENTA DE COMBUSTIBLE
            ...
            if xpos in [1..MaxPosCarga] then begin
              if (TPosCarga[xpos].estatus in [2,9]) then begin
                //DespliegaMemo4('E> E '+inttostr(xpos));
                if DetenerDespacho(xpos) then begin
                  //DespliegaMemo4('R> E '+inttostr(xpos));
                end;
              end;
            end;
          end
            ...
            if xpos in [1..MaxPosCarga] then begin
              if (TPosCarga[xpos].estatus in [8]) then begin
                //DespliegaMemo4('E> G '+inttostr(xpos));
                if ReanudaDespacho(xpos) then begin
                  //DespliegaMemo4('R> G '+inttostr(xpos));
                end;
              end;
            end;
          end
              ...
              if DigitosGilbarco=6 then begin
                ...
                begin
                  for i:=1 to nocomb do begin
                    ...
                    if xp>0 then begin
                      DespliegaMemo4('Pos '+inttostr(PosCiclo)+'    Comb '+inttostr(xcomb)+'    xp '+inttostr(xp)+'     i '+inttostr(i));
                  ...
                  DespliegaMemo4(dmcons.LogTotales);
