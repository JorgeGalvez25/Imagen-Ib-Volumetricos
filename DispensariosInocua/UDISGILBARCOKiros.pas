...
type
     ...
     regprecios = record
       SwCambPrecN1:array[1..4] of boolean;
       SwCambPrecN2:array[1..4] of boolean;
...
// Cadena inicio: 7E  XX  03  01  00
// mensaje gilbarco 2wire
// Cadena fin:    CRC8 7E 
var ss:string;
    i:integer;
    ch:char;
begin
  ...
  // INICIA
  i:=1;   // Analisis de la cadena para identificar posibles caracteres de control: $7E $7D
  repeat
  ...
  // fin
  ss:=char(TPosCarga[xpos].Canal)+char(3)+char(1)+char(0)+xss;   // concatena la cadena inicial
  ch:=char(crc8(ss));
  if ch<>char($7E) then
    ss:=ss+ch
  else
...
begin
   ...
   repeat
      ...
      xComando:=EmpacaKairos(chComando,xnpos);          // Empaca mensaje
      ...
      if ( not bOk ) then begin
         ...
         if ( bListo ) then begin
            sRespuesta:=DesEmpacaKairos(sRespuesta);           // Desempaca mensaje
            if TPosCarga[xpos].DigitosGilbarco=6 then begin
              if ( iComando=$00 ) then
         ...
         if ( not bOk ) then begin
            ...
            PuertoSerial.FlushInBuffer;
            PuertoSerial.FlushOutBuffer;
            xDataBlock:=EmpacaKairos(sDataBlock,xnpos);    // Empaca mensaje
...
begin
   ...
   if ( ( TransmiteComando($00,PosCarga,'') ) and ( length(sRespuesta)>=1 ) ) then case ( HiNibbleChar(sRespuesta[1]) ) of
        $6     : iStatus:= 1;   // inactivo
        $1,$9  : iStatus:= 2;   // despachando
     $A,$B     : iStatus:= 3;   // Fin de venta
        $0     : iStatus:= 0;    // sin comunicacion
        $7     : iStatus:= 5;    // pistola levantada
       $C      : iStatus:= 8;    // detenida
        $8     : iStatus:= 9;    // autorizado
   end;
   result:= iStatus;
end;
  ...
  with DMCONS do begin
    ...
    for i:=1 to 16 do with TPrecios[i] do begin
      for j:=1 to 4 do begin
        SwCambPrecN1[j]:=false;
        SwCambPrecN2[j]:=false;
        NuevoPrec[j]:=0;
      end;
    end;
        ...
        with TPosCarga[xpos] do begin
          ...
          if not existe then begin
            ...
            if Q_BombIbCon_Posicion.AsInteger>0 then begin
              ...
              DMCONS.AgregaLogCmnd('>>NoComb '+inttostr(NoComb)+'    Comb '+inttostr(xcomb)+'    xPos '+inttostr(TPosx[NoComb]));
            end
...
procedure TFDISGILBARCOKiros.Button4Click(Sender: TObject);
var
  ruta:PAnsiChar;
begin
        ...
        for xcomb:=1 to MaxComb do if TabComb[xcomb].Activo then begin
          ...
          for xpos:=1 to MaxPosCarga do with TPrecios[xpos] do
            for xp:=1 to TPosCarga[xpos].NoComb do if xcomb=CombustibleEnPosicion(xpos,xp) then begin
              TPrecios[xpos].SwCambPrecN1[xp]:=true;
              TPrecios[xpos].SwCambPrecN2[xp]:=true;
        ...
        case NumPaso of
          ...
          7:begin          // CAMBIA PRECIO
              ...
              while (xp<=NoComb)and(not swprec) do begin
                if (estatus=1)and((TPrecios[PosCiclo].SwCambPrecN1[xp])or(TPrecios[PosCiclo].SwCambPrecN2[xp])) then
                  SwPrec:=true
                else inc(xp);
              end;
              if swprec then begin
                if TPosCarga[PosCiclo].DigitosGilbarco=6 then begin
                  ...
                  if TPrecios[PosCiclo].SwCambPrecN1[xp] then begin
                    if CambiaPrecio6(PosCiclo,xp,1,TPrecios[PosCiclo].NuevoPrec[xp]) then
                      TPrecios[PosCiclo].SwCambPrecN1[xp]:=false;
                  end
                  else if TPrecios[PosCiclo].SwCambPrecN2[xp] then begin
                    if CambiaPrecio6(PosCiclo,xp,2,TPrecios[PosCiclo].NuevoPrec[xp]) then
                      TPrecios[PosCiclo].SwCambPrecN2[xp]:=false;
                  end;
                end
                else begin
                  DMCONS.AgregaLog('E> Cambia Precio(8): '+inttoclavenum(PosCiclo,2)+' - '+inttoclavenum(xp,2));
                  if TPrecios[PosCiclo].SwCambPrecN1[xp] then begin
                    if CambiaPrecio8(PosCiclo,xp,1,TPrecios[PosCiclo].NuevoPrec[xp]) then
                      TPrecios[PosCiclo].SwCambPrecN1[xp]:=false;
                  end
                  else if TPrecios[PosCiclo].SwCambPrecN2[xp] then begin
                    if CambiaPrecio8(PosCiclo,xp,2,TPrecios[PosCiclo].NuevoPrec[xp]) then
                      TPrecios[PosCiclo].SwCambPrecN2[xp]:=false;
                  end;
                end;
              end;
        ...
        if (NumPaso=7) then begin
          ...
          for xpos:=1 to MaxPosCarga do
            for xp:=1 to TPosCarga[xpos].NoComb do
              if (TPrecios[PosCiclo].SwCambPrecN1[xp])or(TPrecios[PosCiclo].SwCambPrecN2[xp]) then
                swprec:=true;
          if not swprec then
            NumPaso:=8;
        end;
