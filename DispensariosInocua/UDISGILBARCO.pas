   ...
   if ( iComando in [$10,$30,$F0] ) then begin
      ...
      if ( iComando in [$00,$20] ) then begin
         ...
         if ( iComando in [$20] ) then begin
           sw2:=true;           // voy a mandar data block
         end;
      end
      else begin
         iMaxIntentos:= 2;
   ...
   repeat
      ...
      if ( not bOk ) then begin
         ...
         if ( not bOk ) then begin
            ...
            DMCONS.AgregaLog('E '+chComando+' - '+IntToHex($00,1)+'.'+IntToStr(xNPos));
            PuertoSerial.PutChar(chComando);
            repeat
               PuertoSerial.ProcessCommunications;
            until ( PuertoSerial.OutBuffUsed=0 );
...
1 Event  * despachando
6 Off   *idle
7 Call      * pistola levantada
8 Authorized   * autorizado
9 Busy    * despachando
A Transaction Complete at Preset       * Finv
B Transaction Complete Fill-up          *finv
C Pump Stop                             + detenida
D Send Data
(Only returned in response to
<DATA NEXT> Command)
   *)
begin
   iStatus:= 0;
                    ...
                    if TCambioPrecN1[xp] then begin
                      if TPosCarga[PosCiclo].DigitosGilbarco=6 then begin
                        ...
                        if CambiaPrecio6(PosCiclo,xp,1,TNuevoPrec[xp]) then begin
                          Esperamiliseg(200);
                          if CambiaPrecio6(PosCiclo,xp,2,TNuevoPrec[xp]) then begin
                        ...
                        if CambiaPrecio8(PosCiclo,xp,1,TNuevoPrec[xp]) then begin
                          Esperamiliseg(200);
                          if CambiaPrecio8(PosCiclo,xp,2,TNuevoPrec[xp]) then begin
                            TCambioPrecN1[xp]:=false;
                          end;
                        end;
                      end;
