...
function TFDISHONGJANG.ComandoA(xaddr,xlado:integer):string; // Lee venta o display    comando 15 (0F)
// Send  01      04    01       0F       E9
//     address length hose   Command +[parameter]+ parity check bit
var ss:string;
begin
  ss:=char(xaddr)+char(4)+char(xlado)+char(15);
  result:=ss+CalculaBCC(ss);
end;

function TFDISHONGJANG.ComandoC(xaddr,xlado:integer):string; // Enllava      comando 21 (15)
// Send  01     04    01    15  E9
...
begin
  ss:=char(xaddr)+char(4)+char(xlado)+char(21);
  result:=ss+CalculaBCC(ss);
end;

function TFDISHONGJANG.ComandoD(xaddr,xlado:integer):string; // DesEnllava       COMANDO 20 (14)
// Send  01 04 01 14 E9
//       xaddr xlong xlado cmnd          )
var ss:string;
begin
  ss:=char(xaddr)+char(4)+char(xlado)+char(20);
  result:=ss+CalculaBCC(ss);
end;

function TFDISHONGJANG.ComandoN(xaddr,xlado:integer):string; // Lee totalizador    COMANDO 14 (0E)
// CPU=1   LADO=1
// Send  01 04 01 0E EA
//       xaddr xlong xlado cmnd
var ss:string;
begin
  ss:=char(xaddr)+char(4)+char(xlado)+char(14);
  result:=ss+CalculaBCC(ss);
end;
...
var ss:string;
begin
  ss:=char(xaddr)+char(4)+char(xlado)+char(12);
  result:=ss+CalculaBCC(ss);
end;
...
//                IMPORTE=0017.64      LITROS=0013.07     length + state + data + parity check bit
//           0C  12    00 40 00    00 00 00   92 01 00 0F     ahead 3 means price (low bit in the front), middle 3 byte no use, last 3 byte means liters (low bit in the front)
var ss:string;
    ee,xp,ne:integer;
    ximp,xvol,xpre:real;
begin
  with TMangueras[MangCmnd] do begin
    ...
    try
      ...
      DMCONS.AgregaLogCmnd('Error BCD: '+xresp);
    end;
  end;
end;
    ...
    if (Now-DMCONS.FechaHoraPrecio)>12*tmSegundo then begin
      ...
      with DMCONS do if AplicarPrecios then begin // checa si hay precio pendiente de aplicar
        for i:=1 to MaxComb do with TabComb[i] do if Activo then begin
          ...
          RegistraBitacoraCP(i);                           // cambia estatus del precio a aplicado
        end;
