...
begin
  if SwInicio then begin
    ...
    try
      ...
      if (DMCONS.MapeoFusion='Si') then begin
...
Reintenta:
              ...
              if (DMCONS.WayneFusionCombXPrecio='Si') then begin // Checo combustible por el precio
           ...
           for xpos:=1 to length(ss) do if xpos in [1..MaxPosCarga] then begin
             with TPosCarga[xpos] do if Estatus=1 then begin
               ...
               if RefrescaEnllavados then begin
                 ...
                 //if (DMCONS.WayneFusion='No')or(DMCONS.MapeoFusion='Si') then begin
                 if (DMCONS.MapeoFusion='Si') then begin
                   ComandoConsolaBuff('h'+IntToClaveNum(xpos,2)+'00');      // Eliminar el autoautoiza
                   esperamiliseg(200);
                   ComandoConsolaBuff('k'+IntToClaveNum(xpos,2)+'00');      // Eliminar el autopago
                   esperamiliseg(200);
                   MapeaPosicion(xpos);
                   esperamiliseg(200);
                   exit;
             ...
             with TPosCarga[xpos] do begin
               case Estatus of
                 5:if (ModoOpera='Normal')and(not SwDesHabilitado)and(not swcargando) then begin
                     ...
                     ComandoConsola('S'+IntToClaveNum(xpos,2)+'0'+'0');    // Cualquier precio  -  Todas las mangueras
                     esperamiliseg(200);
                     (*
                     if DMCONS.swemular then
                       EmularEstatus[xpos]:='2';                S
                       *)
                   end;
               end;
             end;
               ...
               if T_Tcmb.Locate('Clave',PrecioFisicoProc,[]) then begin
                 ...
                 *)
                 ...
                 if (abs(xprec-TabComb[i].PrecioNuevo)<0.001)or(abs(TabComb[i].PrecioNuevo)<0.001) then begin
                   ...
                   T_TcmbPrecioFisico.AsFloat:=TabComb[i].PrecioNuevo;   
                 end;
                 T_Tcmb.Post;
               end;
             finally
    ...
    if NumPaso=5 then begin // TOTALES
      if PosicionCargaActual2<=MaxPosCarga then begin
        repeat
          ...
          else if PosicionDispenActual<4 then 
