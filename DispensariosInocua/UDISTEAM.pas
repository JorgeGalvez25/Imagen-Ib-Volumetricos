...
type
  ...
  public
    ...
    function ComandoA(xdisp,xlado:integer):string;
               ...
               if DMCONS.ValidaCifra(TabComb[xcomb].precio,2,2)='OK' then begin
                 if TabComb[xcomb].precio>=0.01 then begin
                   ...
                   if NoComb=1 then begin
                     ss:=ss+'0000'+'0000';    //inttoclavenum(ii,4)+inttoclavenum(ii,4);
                   end
           ...
           if (xpos>=1)and(xpos<=DMCONS.MaximoDePosiciones) then begin
             ...
             with TPosCarga[xpos] do begin
               try
                 ...
                 swinicio2:=false;
                 volumen:=StrToFloat(copy(lin,5,7))/1000;
                 simp:=copy(lin,12,8);
                 spre:=copy(lin,20,4);
                 ...
                 *)
                 ...
                 DMCONS.AgregaLogCmnd(lin+' '+fechapaq(date)+' '+HoraPaq(time));
               end;
             end;
           end;
         end;
...
//  A3    DD    00    02    LL     &&
//  CMD   DISP  SEP  LONG   DATO   BDC            Nota: LONG = bytes que vienen despues incluyendo BDC
begin
  result:='A3 '+inttoclavenum(xdisp,2)+' 00 02 '+inttoclavenum(xlado,2);
end;


function TFDISTEAM.ComandoA(xdisp,xlado:integer):string; // Leer display
//  A1    ##    00    03    LL     TT       &&
//  CMD   DISP  SEP  LONG   DATO   TIPO     BDC
begin
  result:='A1 '+inttoclavenum(xdisp,2)+' 00 03 '+inttoclavenum(xlado,2)+' 02';
end;

function TFDISTEAM.ComandoS(xdisp,xlado:integer;ximpo:real):string; // Presetear Moneda
//  A5    ##     00    07    LL     06   99 99 99 99    &&
//  CMD   DISP  SEP  LONG   DATO   MODO   IMPORTE      BDC                             MODO 06: POR IMPORTE
var ss,ss2:string;
    ii:integer;
begin
  ss:='A5 '+inttoclavenum(xdisp,2)+' 00 07 '+inttoclavenum(xlado,2)+' 06';
  if DMCONS.DecimPresetTEAM='Si' then
    ii:=trunc(ximpo+0.5)
  else
    ii:=trunc(ximpo*100+0.5);
  ss2:=inttoclavenum(ii,8);
  result:=ss+' '+copy(ss2,1,2)+' '+copy(ss2,3,2)+' '+copy(ss2,5,2)+' '+copy(ss2,7,2);
end;


function TFDISTEAM.ComandoL(xdisp,xlado:integer;xlitros:real):string; // Presetear Litros
//  A5    ##     00    07    LL     09   99 99 99 99    &&
//  CMD   DISP  SEP  LONG   DATO   MODO   VOLUMEN       BDC                             MODO 06: POR IMPORTE
var ss,ss2:string;
    ii:integer;
begin
  ss:='A5 '+inttoclavenum(xdisp,2)+' 00 07 '+inttoclavenum(xlado,2)+' 09';
  if DMCONS.DecimPresetTEAM='Si' then
    ii:=trunc(xlitros+0.5)
  else
    ii:=trunc(xlitros*1000+0.5);
  ss2:=inttoclavenum(ii,8);
  result:=ss+' '+copy(ss2,1,2)+' '+copy(ss2,3,2)+' '+copy(ss2,5,2)+' '+copy(ss2,7,2);
end;

function TFDISTEAM.ComandoU(xdisp:integer; xprec1,xprec2,xprec3:real):string;  // Cambio precios
//  A6    ##     00    08    06    11 11     22 22     33 33    &&
//  CMD   DISP  SEP  LONG   FIJO   PREC1     PREC2     PREC3    BDC
var ss,ss2:string;
    ii:integer;
begin
...
//  A9    ##    00    03    LL     PP       &&
//  CMD   DISP  SEP  LONG   DATO   PROD     BDC
begin
  result:='A9 '+inttoclavenum(xdisp,2)+' 00 03 '+inttoclavenum(xlado,2)+' '+inttoclavenum(xprod+10,2);
end;

function TFDISTEAM.ComandoC(xdisp:integer):string; // Bloquea
//  A0    ##    00    02    06      &&
//  CMD   DISP  SEP  LONG   FIJO    BDC
var ss:string;
begin
  ...
  *)
  ...
  case CharCmnd of
    ...
    'A':begin
          ...
          //n:=strtointdef(s1,0);
          if xpos in [1..MaxPosCarga] then with TPosCarga[xpos] do begin
            LinCmd2:=ComandoA(dispensario,lado);
            (*
            case n of
...
begin
  ...
  try
    ...
    while (not FinLinea)and(Length(LineaBuff)>0) do begin
      ...
      if (ss[1]='A') then // INICIA UN COMANDO
        linea:='';
      delete(LineaBuff,1,1);
      Linea:=Linea+C;
      LineaTimer:=StrToHexSep(Linea);
      if (copy(LineaTimer,1,2)='A3')and(noelemstrsep(LineaTimer,' ')=9) then           // A3 ## 00 05 LL SS PP CC &&
        FinLinea:=true
      else if (copy(LineaTimer,1,2)='A1')and(noelemstrsep(LineaTimer,' ')=14) then     // A1 ## 00 07 LL UU 99 99 99 99 &&
        FinLinea:=true                                                                 // A1 ## 00 10 LL VV VV VV VV II II II II &&
    ...
    if FinLinea then begin
      ...
      FinLinea:=false;
      //   A3    ##    00    05     LL     SS     PP     CC     &&
      //   CMD  DISP  SEP   LONG   LADO   STAT  PROD   CRED    BDC
      if copy(LineaTimer,1,2)='A3' then begin  // Comando B  : Lee estatus
        if not csok then
          checksumb:=false;
        HoraRspB:=now;
        ...
        with TPosCarga[xpos] do begin
          ...
          (*
          Estatus del dispensario, valor en hexadecimal, estatus por bit.
          bit 1: Siempre 0.                  1
          bit 2: Autoservicio.               2
          bit 3: Prefijado activo.           3
          bit 4: Prefijado en importe.       4
          bit 5: Siempre cero.               5
          bit 6: Prefijado en volumen.       6            40  0100 0000
          bit 7: Dispensario ocupado.        7
          bit 8: Error en memoria.           8
          *)
          if csok then begin
            ...
            case s1[7] of
              '0':begin
                    estatus:=1; // Inactivo
                    if (s1[3]='1')or(s1[4]='1')or(s1[6]='1') then
                      estatus:=2; // Autorizado
                  end;
              '1':estatus:=5; // Despachando
            end;
            if s1[8]='1' then    // Error
              estatus:=9;
      ...
      //   A1    ##    00    07     LL    UU       99 99 99 99     &&
      //   CMD  DISP  SEP   LONG   LADO   MON/VOL    VALOR        BDC
      //   A1    ##    00    10     LL    VV VV VV VV   II II II II  &&
      //   A1    01    00    10     01    00 00 30 00   00 00 30 00 61
      else if copy(LineaTimer,1,2)='A1' then begin // COMANDO A   LEE VENTA
        xpos:=DamePosTeam(LineaTimer,true);
        with TPosCarga[xpos] do begin
            try
              ss:=ExtraeElemStrSep(LineaTimer,6,' ')
                 +ExtraeElemStrSep(LineaTimer,7,' ')
                 +ExtraeElemStrSep(LineaTimer,8,' ')
                 +ExtraeElemStrSep(LineaTimer,9,' ');
              if (ss<>'FFFFFFFF')and(ss<>'88888888') then begin
                if DMCONS.TresDecimVolTEAM='Si' then
                  volumen:=strtoint(ss)/1000
                else
                  volumen:=strtoint(ss)/100;
              end;
              ss:=ExtraeElemStrSep(LineaTimer,10,' ')
                 +ExtraeElemStrSep(LineaTimer,11,' ')
                 +ExtraeElemStrSep(LineaTimer,12,' ')
                 +ExtraeElemStrSep(LineaTimer,13,' ');
              ...
              LineaTimer:='A'+inttoclavenum(xpos,2)+'0'
                           +FormatFloat('0000000',volumen*1000)
      ...
      //   A5    ##    00    07     LL    UU       99 99 99 99     &&
      //   CMD  DISP  SEP   LONG   LADO   MON/VOL    VALOR        BDC       Nota: UU=06 Importe   UU=09  Litros
      else if copy(LineaTimer,1,2)='A5' then begin // COMANDO S  PREFIJAR LITROS O PESOS
        xpos:=DamePosTeam(LineaTimer,true);
        ss:=ExtraeElemStrSep(LineaTimer,6,' ');
        if ss='09' then
      ...
      //   A6   ##     00     08    06     11 11   22 22       33 33       &&
      //   CMD  DISP  SEP   LONG   LADO   PREC P1  PRECIO P2   PRECIO P3  BDC
      ...
      //  A9    ##    00    08     LL    VV VV VV VV VV VV       &&
      //  CMD  DISP  SEP   LONG   LADO   VOLUMEN                BDC
  ...
  try
    ...
    if SwComandoB then begin
      ...
      if PosCiclo>MaxPosCarga then begin
        PosCiclo:=1;
        // arama la respuesta en modalidad 515   B00 11 21 11 11
        LineaTimer:='B00';
        CheckSumB:=true;
        for xpos:=1 to MaxPosCarga do
          with TPosCarga[xpos] do begin
