                ...
                Case DMCONS.TipoClb[1] of
                  '2':begin
                    ...
                    if (Estatus=1)and(Stflu=1)and(swflu) then begin // Manda Flu SUBE
                      // Pasada magna-only (HD in [0,1,2]): not EsDiesel AND not EsMixto â†’ 957.3x
                      // Pasada EsMixto    (HD=5)          : EsMixto=True              â†’ 959.xy
                      // Pasada diesel puro(HD=3)          : EsDiesel=True             â†’ 957.4x
                    ...
                    if (PosFlu=PosCiclo)and(stflu in[2,12]) then begin // detener flu
                      if (estatus in[2,9]) then begin
                        ...
                        StFlu:=0;
                        Case HayDiesel of
                          // Pasada magna terminada â†’ EsMixto si hay, sino diesel
                          2:begin
                          ...
                          // Pasada EsMixto terminada â†’ diesel si hay, sino fin
                          5:begin
                              xp:=0;
                              for i:=1 to MaxPosCarga do
                          ...
                          // Pasada baja magna terminada â†’ EsMixto baja si hay, sino diesel baja
                         -2:begin
                              if HayMixto=1 then begin
                          ...
                          // Pasada baja EsMixto terminada â†’ diesel baja si hay, sino fin
                         -5:begin
                              xp:=0;
                              for i:=1 to MaxPosCarga do
                                if TPosCarga[i].EsDiesel then xp:=1;
