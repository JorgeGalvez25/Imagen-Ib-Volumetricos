...
type
  ...
  public
    ...
    procedure EnviaPreset3(var rsp:string;xcomb:integer;xprec:string;validaestat:boolean);
    procedure EnviaPreset(var rsp:string;xcomb:integer;xprec:string);
...
function TFDISPAM10002.DigitosDispensario(xpos:integer):integer;
begin
...
label uno;
var lin,ss,rsp,
    xestado,xmodo,sprec:string;
    simp,sval,spre:string[20];
    i,xpos,xcmnd,
    XMANG,XCTE,XVEHI,
    xcomb,xc,xfolio:integer;
    xgrade:char;
    importeant,
    ximporte:real;
    xvol,ximp:real;
                   ...
                   case Estatus of
                     ...
                     5:if (not SwDesHabilitado)and(not swautorizada) then begin
                         ...
                         if (ModoOpera='Normal')and(not swarosmag) then begin
                           ...
                           else if (DMCONS.ModoAutorizaBennett=0)or(DMCONS.VersionPam1000<>'3') then begin
                           ...
                           apeg:=18;
                           if (DMCONS.ModoAutorizaBennett=0)or(DMCONS.VersionPam1000<>'3') then begin
                             ss:='S'+IntToClaveNum(xpos,2); // AUTHORIZATION FOR FILLUP
                             ComandoConsola(ss);
                             SwAutorizando:=true;
                           end
            ...
            if not swreset then with TabComb[PrecioCombActual] do begin
              if AplicaPrecio then begin // Cambia Precios
                if ProductoPrecio<>'' then begin
                  ComandoConsola('X'+'00'+ProductoPrecio+'1'+'00'+IntToClaveNum(Trunc(Precio*100+0.5),4));
                  EsperaMiliSeg(300);
                  if SwEmular then
                    AplicaPrecio:=false;
                  exit;
                        ...
                        if rsp='OK' then begin
                          if (TPosCarga[SnPosCarga].estatus in [1,5,9]) then begin
                            ...
                            if DMCONS.VersionPam1000='3' then
                              EnviaPreset3(rsp,xcomb,sprec,true)
                            else
                              EnviaPreset(rsp,xcomb,sprec);
                          end
                          else begin
...
begin
  ...
  SwEsperaRsp:=false;
  SwComandoB:=false;
  if DMCONS.VersionPam1000='11' then
    StaticText7.Caption:='PAM 1000 /11'
  else if DMCONS.VersionPam1000='3' then
    StaticText7.Caption:='PAM 1000 / 3';
end;
...
procedure TFDISPAM10002.EnviaPreset3(var rsp:string;xcomb:integer;xprec:string;validaestat:boolean);
...
begin
  ...
  NivelPrec:='1';
  with DMCONS,TPosCarga[xpos] do if xprec<>'' then begin // precios especiales
    CargaPreciosEspeciales(xprec);
    for xp:=1 to 4 do begin
      if TabPrecEsp[xp]>0.01 then begin
        xc:=CombustibleEnPosicion(xpos,xp);
        // Envia Precio Especial
        ComandoConsola('X'+'00'+TabComb[xc].ProductoPrecio+'2'+'00'+IntToClaveNum(Trunc(TabPrecEsp[xp]*100+0.5),4));
        EsperaMiliSeg(200);
        NivelPrec:='2';
      end;
    end;
  end;
  TPosCarga[xpos].SwPresetManual:=false;
  xprodauto:='000000';
  with TPosCarga[xpos] do begin
...
procedure TFDISPAM10002.EnviaPreset(var rsp:string;xcomb:integer;xprec:string);
    ...
    if DigitosDispensario(xpos)=5 then begin
      if not swlitros then begin // PRESET EN IMPORTE
        case DMCONS.DecimalesPresetPAM of
          0:ss:='P'+IntToClaveNum(xpos,2)+'0'+NivelPrec+'000'+FiltraStrNum(FormatFloat('00000',snimporte))+'0';
          1:ss:='P'+IntToClaveNum(xpos,2)+'0'+NivelPrec+'000'+FiltraStrNum(FormatFloat('0000.0',snimporte))+'0';
          else begin
      ...
      if not swlitros then begin // PRESET IMPORTE
        ...
        ss:='P'+IntToClaveNum(xpos,2)+'0'+NivelPrec+'000'+FiltraStrNum(FormatFloat('00000',snimporte))+'0';
      end
...
begin
  ...
  ComandoConsola('@070');
  SwReset:=true;
  StaticText8.Caption:='Reseteando..';
  ContReset:=0;
end;
...
procedure TFDISPAM10002.Timer2Timer(Sender: TObject);
begin
  if DMCONS.StLevanta='Si' then begin
    if (Now-HoraTimer1)>20*tmsegundo then begin
      DMCONS.AgregaLog('Aborta Aplicacion!!');
      DMCONS.ListaLog.SaveToFile('\ImagenCo\Log'+FiltraStrNum(FechaHoraToStr(Now))+'.Txt');
      Ap1.Open:=false;
      Application.Terminate;
    end;
