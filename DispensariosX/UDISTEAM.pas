...
procedure TFDISTEAM.ActualizaAdic(xval:integer);
begin
  ...
  with DMCONS do begin
    ...
    while not Q_BombIb.Eof do begin
      ...
      with TPosCarga[xpos] do begin
        ...
        SwA:=false;
        existe:=false;
        TCodigoTeam:=Q_BombIbImpreTarjetas.AsString;
          ...
          if (importe>0.001)or(tipopago>0) then begin
            ...
            try
              try
                ...
                if TPosCarga[i].swflujovehiculo then begin
                  lin:=TPosCarga[i].cmndflujo;
          ...
          if not T_ConfIb.IsEmpty then begin
            ...
            try
              if dmcons.MaximoDePosiciones<24 then begin
                // xdisp2:=T_ConfIbDispensarios.AsString;
