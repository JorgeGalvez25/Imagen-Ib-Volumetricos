...
type
  ...
  public
    ...
    LcLicencia2:string;
    LcLicTemporal2:boolean;
    LcLicVence2:TDateTime;
    StLevanta:string;
    LcSerie:integer;
    PtParidad   :string[1];
    PtPuerto,
    ...
    procedure CargaPreciosEspeciales(xprec:string);
...
uses UIMGUTIL, DECUTIL, Cipher; // ULIBLICENCIAS,
...
begin
  ...
  try
    ...
    try
      ...
      for i:=0 to Lista.Count-1 do begin
        if i=0 then
          ...
          tadic31[i]:=StrToFloat(lista[i]);
      end;
    except
    end;
    Lista.Clear;
    try
      Lista.LoadFromFile(DirectorioWindows+'\AdicTmp2.Txt');
      for i:=0 to Lista.Count-1 do begin
        if i=0 then
          stadic:=strtointdef(lista[0],0)
        else if i in [1..32] then begin
          tadic32[i]:=StrToFloat(lista[i]);
          if tadic32[i]<-0.1 then
            tadic32[i]:=tadic31[i];
        end
      end;
    except
    ...
    Lista.Clear;
    try
      Lista.LoadFromFile(DirectorioWindows+'\AdicTmp3.Txt');
      for i:=0 to Lista.Count-1 do begin
        if i=0 then
          stadic:=strtointdef(lista[0],0)
        else if i in [1..32] then begin
          tadic33[i]:=StrToFloat(lista[i]);
          if tadic33[i]<-0.1 then
            tadic33[i]:=tadic31[i];
        end;
      end;
    except
...
procedure TDMCONS.CargaPreciosEspeciales(xprec:string);
var ss:string;
    xpos:integer;
begin
  for xpos:=1 to 4 do
    TabPrecEsp[xpos]:=0;
  try
    for xpos:=1 to NoElemStrSep(xprec,';') do begin
      ss:=ExtraeElemStrSep(xprec,xpos,';');
      if xpos in [1..4] then begin
        TabPrecEsp[xpos]:=StrToFloat(ss);
        if TabPrecEsp[xpos]<0 then
          TabPrecEsp[xpos]:=0;
      end;
    end;
  except
...
begin
  ...
  try
    ...
    for i:=0 to lista.Count-1 do begin
      ...
      if Mayusculas(ss)='MODELODISPENSARIOENCORE' then
        ModeloDispensarioEncore:=ExtraeElemStrSep(lin,2,'=');
      ...
      if Mayusculas(ss)='MAXIMOPRESETPAM' then
        MaximoPresetPam:=StrToIntDef(ExtraeElemStrSep(lin,2,'='),9999);
      ...
      if Mayusculas(ss)='POSTARJETA2' then
        PosTarjeta2:=StrToIntDef(ExtraeElemStrSep(lin,2,'='),0);

      // VARIABLES BENNETT
      if Mayusculas(ss)='FLUJOPORVEHICULO' then
        FlujoPorVehiculo:=ExtraeElemStrSep(lin,2,'=');
      ...
      if Mayusculas(ss)='BENNETTPRESETEXTENDIDO' then
        BennettPresetExtendido:=ExtraeElemStrSep(lin,2,'=');
      if Mayusculas(ss)='BENNETTOMITIRCOMANDOF' then
      ...
      if Mayusculas(ss)='BENNETTNIVELPRECIO' then begin
        ...
        while length(BennettNivelPrecio)<2 do
          BennettNivelPrecio:=BennettNivelPrecio+'X';
      end;
      if Mayusculas(ss)='BENNETTPROTEC' then begin
        BennettProtec:=ExtraeElemStrSep(lin,2,'=');
      ...
      if Mayusculas(ss)='VALORTEAM1' then
        ValorTeam1:=StrToIntDef(ExtraeElemStrSep(lin,2,'='),5);
      if Mayusculas(ss)='VALORTEAM2' then
        ValorTeam2:=StrToIntDef(ExtraeElemStrSep(lin,2,'='),5);
      if Mayusculas(ss)='VALORTEAM3' then
        ValorTeam3:=StrToIntDef(ExtraeElemStrSep(lin,2,'='),5);
      if Mayusculas(ss)='CODIGOTEAM' then
        CodigoTeam:=ExtraeElemStrSep(lin,2,'=');
      ...
      if Mayusculas(ss)='VALORHONGYANG' then begin
        ValorHongYang:=StrToIntDef(ExtraeElemStrSep(lin,2,'='),5);
        if ValorHongYang>6 then
          ValorHongYang:=6;
      end;
...
begin
  ...
  lcLicTemporal2:=T_ConfIbEsTemporal2.AsString='Si';
  lcLicVence2:=T_ConfIbFechaVence2.AsDateTime;
...
var xhor,xhori,xhorf,xhormin,xhormax,fec1,fec2:string;
    ...
    swhoy,swmanana:boolean;
begin
  swhoy:=false;swmanana:=false;
  margenant:=3/24;Margenpos:=3/24;
  xhor:=FiltraStrNum(HoraToStr24(xFechaHora));
  T_Turc.Active:=false;
  T_Turc.TableName:='DPVGTURC';
  if T_ConfIbUsaTurnosAlternativos.AsString='Si' then begin
    // Verifica si hay cambio de precios al hoy
    try
      fec1:=fechatostrsql(date);
      fec2:=fechatostrsql(date+1);
      try
        Q_Auxi.Active:=false;
        Q_AuxiEntero1.FieldKind:=fkInternalCalc;
        Q_Auxi.SQL.Clear;
        Q_Auxi.SQL.Add('Select count(*) as Entero1 from DPVGCPRE');
    ...
    // fin verifica hoy
    // Verifica si hay cambio de precios mañana
    if not swhoy then begin
      try
        fec1:=fechatostrsql(date+1);
        fec2:=fechatostrsql(date+2);
        try
          Q_Auxi.Active:=false;
          Q_AuxiEntero1.FieldKind:=fkInternalCalc;
          Q_Auxi.SQL.Clear;
    ...
    // fin verifica manana
    if (swhoy)and(time<0.25)or(swmanana)and(time>0.75) then
      T_Turc.TableName:='DPVGTURCA';
  end;
  T_Turc.Active:=true;
