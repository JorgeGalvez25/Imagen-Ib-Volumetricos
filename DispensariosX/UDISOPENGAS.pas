...
procedure TFDISOPENGAS.ActualizaAdic(xval:integer);
begin
...
procedure TFDISOPENGAS.DespliegaMemo4(lin:string);
begin
end;

procedure TFDISOPENGAS.DespliegaPrecios;
var i:integer;
begin
  with DMCONS do begin
    Memo1.Lines.Clear;
...
procedure TFDISOPENGAS.IniciaBaseDeDatos;
var i:integer;
begin
  with DMCONS do begin
    ...
    try
      if not swemular then begin
        ...
        Socket1.Host:=ServidorGateway;
        Socket1.Port:=PuertoGatewayDisp;
        Socket1.Open;
      end;
...
procedure TFDISOPENGAS.IniciaEstacion;
...
procedure TFDISOPENGAS.FormShow(Sender: TObject);
begin
...
function TFDISOPENGAS.StringCom(ss:string):string;
var str:string;
    i:integer;
begin
...
function TFDISOPENGAS.XorChar(c1,c2:char):char;
...
function TFDISOPENGAS.CalculaBCC(ss:string):char;
var xc,cc:char;
    i:integer;
begin
  xc:=ss[1];
...
procedure TFDISOPENGAS.LimpiaConsola;
begin
  StaticText1.Caption:='  ';
  StaticText2.Caption:='  ';
  StaticText3.Caption:='  ';
...
procedure TFDISOPENGAS.DespliegaPosCarga(xpos:integer;swforza:boolean);
...
begin
  ...
  try
    ...
    posi:=rango*4-3;
    posf:=rango*4;
    with TPosCarga[xpos],FDISOPENGAS do begin
          ...
          try
            try
              ...
              if (lcLicTemporal)and(date>lcLicVence) then begin
                ...
                swcerrar:=true; FDISOPENGAS.Close;
              end;
            finally
...
procedure TFDISOPENGAS.ProcesaLinea;
label uno;
var lin,ss,rsp,ss2,
                 ...
                 if lin[4]='0' then begin // POSICION ESTA CARGANDO
                   ...
                   for i:=1 to MCxP do
                     if xGrade=IntToStr(TComb[i]) then
                       PosActual:=TPosx[i];
                   if (PosActual=0)or(checkbox2.Checked) then begin   // Perdi� el mapeo
                     for i:=1 to nocomb do
                       SwMapea[i]:=true;
                   end
                   else begin
...
procedure TFDISOPENGAS.ComandoConsolaBuff(ss:string);
begin
  ComandoConsola(ss);
end;

procedure TFDISOPENGAS.ComandoConsola(ss:string);
var s1:string;
    cc:char;
begin
  if swreset then
...
procedure TFDISOPENGAS.ListBox1Click(Sender: TObject);
var xp:integer;
begin
  LimpiaConsola;
...
procedure TFDISOPENGAS.Restaurar1Click(Sender: TObject);
begin
  FDISOPENGAS.Visible:=true;
end;

procedure TFDISOPENGAS.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
procedure TFDISOPENGAS.FormCreate(Sender: TObject);
begin
  SwReset:=false;
  SwCerrar:=false;
  SwInicio:=true;
...
procedure TFDISOPENGAS.BitBtn3Click(Sender: TObject);
begin
  Visible:=false;
  NotificationIcon1.Show;
end;

procedure TFDISOPENGAS.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFDISOPENGAS.Timer1Timer(Sender: TObject);
var ss:string;
    i:integer;
begin
  try
...
procedure TFDISOPENGAS.EnviaPreset3(var rsp:string;xcomb:integer);
...
begin
  ...
  if not swlitros then begin // PRESET EN IMPORTE
    ss:='@02'+'0'+IntToClaveNum(xpos,2)+'00'+FiltraStrNum(FormatFloat('0000.00',snimporte))+xprodauto;
    TPosCarga[xpos].ImportePreset:=SnImporte;
    TPosCarga[xpos].MontoPreset:='$ '+FormatoMoneda(SnImporte);
  end
  else begin // PRESET EN LITROS
    ss:='@02'+'0'+IntToClaveNum(xpos,2)+'01'+FiltraStrNum(FormatFloat('0000.00',snlitros))+xprodauto;
...
procedure TFDISOPENGAS.EnviaPreset(var rsp:string;xcomb:integer);
var xpos,xp:integer;
    ss,NivelPrec:string;
    swlitros:boolean;
begin
...
procedure TFDISOPENGAS.Timer3Timer(Sender: TObject);
...
procedure TFDISOPENGAS.registro(valor:integer;variable:string);
var
  Registry: TRegistry;
begin
...
procedure TFDISOPENGAS.NotificationIcon1DblClick(Sender: TObject);
begin
  Restaurar1Click(Sender);
end;

procedure TFDISOPENGAS.lee_registro;
var
  Registry: TRegistry;
  estado:integer;
begin
...
procedure TFDISOPENGAS.Button1Click(Sender: TObject);
begin
  DMCONS.ListaLog.SaveToFile('\ImagenCo\Log'+FiltraStrNum(FechaHoraToStr(Now))+'.Txt');
end;

function TFDISOPENGAS.CombustibleEnPosicion(xpos,xposcarga:integer):integer;
var i:integer;
begin
  with TPosCarga[xpos] do begin
    result:=0;
...
function TFDISOPENGAS.PosicionDeCombustible(xpos,xcomb:integer):integer;
var i:integer;
begin
  with TPosCarga[xpos] do begin
    result:=0;
...
function TFDISOPENGAS.DamePorcientoComb(xcomb:integer):integer;
var xval:integer;
begin
  result:=0;
  try
...
procedure TFDISOPENGAS.Button3Click(Sender: TObject);
begin
  DMCONS.ListaLogCmnd.SaveToFile('\ImagenCo\LogCmnd'+FiltraStrNum(FechaHoraToStr(Now))+'.Txt');
end;

procedure TFDISOPENGAS.Socket1Read(Sender: TObject;
