...
type
  ...
  public
    ...
    function ComandoW(xaddr,xlado,xvalor:integer):string;
    ...
    function DameEstatus(xstr:string;var swerror:boolean):integer;
...
var
  ...
  TimeCmnd      :TDateTime;  // Momento de envio de comando, es para medir la espera
  LinCmndHJ,
  LinCmnd         :string;
  CharCmnd        :char;
  MangCmnd        :integer;
...
procedure TFDISHONGJANG.ActualizaAdic(xval:integer);
begin
...
var i,rango,posi,posf,xcomb,xc:integer;
    ...
    try
      ...
      with TMangueras[xmang] do begin
        ...
        if poscarga in [posi..posf] then begin
          ...
          case estatus of
            5:TPanel(FindComponent(xpanel)).color:=ClLime;
            ...
            7,8:TPanel(FindComponent(xpanel)).color:=ClBlue;
          ...
          if (importe>0.001)or(tipopago>0) then begin
            ...
            try
              try
                ...
                case PosComb of
                  1:T_MoviIbTotal01.AsFloat:=AjustaFloat(TotalLitros,3);
                  2:T_MoviIbTotal01.AsFloat:=AjustaFloat(TotalLitros,3);
                  3:T_MoviIbTotal01.AsFloat:=AjustaFloat(TotalLitros,3);
                  4:T_MoviIbTotal01.AsFloat:=AjustaFloat(TotalLitros,3);
                end;
                ...
                TMangueras[i].TipoPagoAnt:=TMangueras[i].TipoPago;
            ...
            try
              T_ConsIb.Refresh;
              if T_ConsIb.Locate('PosCarga',PosCarga,[]) then begin
                T_ConsIb.Edit;
                // publica valores
                ...
                T_ConsIbOtros.AsString:=FormatFloat('#######0.00',totallitros);
                // fin publica valores
                T_ConsIbEstatus.AsString:='F';
                T_ConsIb.Post;
              end;
...
function TFDISHONGJANG.ComandoW(xaddr,xlado,xvalor:integer):string;
// Densidad 6%, Addr 03, lado 02
// Send  03 06 02 01 00 06 EE
var ss:string;
begin
  ss:=char(xaddr)+char(6)+char(xlado)+char(1)+ConvierteBCD(xvalor,4);
  result:=ss+CalculaBCC(ss);
end;

procedure TFDISHONGJANG.ComandoConsola(ss:string);
var s1,s2:string;
    xmang,xprecio,ximporte,xlitros,xvalor:integer;
begin
  LinCmnd:=ss;
  ...
  if (MangCmnd>=1)and(MangCmnd<=MaxMangueras) then begin
    ...
    case Charcmnd of
      ...
      'W':begin
            xmang:=strtointdef(copy(LinCmnd,2,2),0);
            xvalor:=strtointdef(copy(LinCmnd,4,2),0);
            if xmang in [1..MaxMangueras] then with TMangueras[xmang] do
              LinCmndHJ:=ComandoW(address,lado,xvalor);
          end;
      else exit;
    end;
    Inc(TotalTramas);
...
function TFDISHONGJANG.DameEstatus(xstr:string;var swerror:boolean):integer;
//  [5] Motor encendido
//  [2] Pistola levantada
var xst,ss:string;
    ee:integer;
...
begin
  with TMangueras[MangCmnd] do begin
    ...
    try
      ...
      if (estatus=1)and(finventa=1)and(swfinventa) then
        estatus:=7;
      with DMCONS do if ((estatusant<>1)and(estatus=1))or(SwAdic) then begin
        ...
        if true then begin //(Licencia2Ok or Licencia3Ok) then begin
          if stadic=2 then // Activado
            ComandoConsola('W'+IntToClaveNum(MangCmnd,2)+IntToClaveNum(DMCONS.ValorHongYang,2))
          else
            ComandoConsola('W'+IntToClaveNum(MangCmnd,2)+IntToClaveNum(0,2));
        end;
      end;
      case estatus of
        1:begin  // Inactivo
      ...
      if (Estatus in [1,7])and(swcargando) then begin
        swcargando:=false;
        swdesp:=true;
        if TMangueras[MangCmnd].SwPrepagoM then begin
          ComandoConsola('C'+inttoclavenum(MangCmnd,2));
          esperamiliseg(100);
        end;
      end;
...
begin
  try
        ...
        'A':begin
              if MangCiclo<=MaxMangueras then with TMangueras[MangCiclo] do begin
                if ContParo=0 then begin
                  if (ContBrinca=0)or(Estatus<>1) then begin
                    if Estatus=1 then
                      ContBrinca:=2;
    ...
    // MANEJO DE ESPERA
      ...
      if (Now-TimeCmnd)>TmSegundo then begin
        SwEsperaCmnd:=false;
        if LineaProc<>'' then
          ProcesaLineaRec(LineaProc)
        else with TMangueras[MangCmnd] do begin
          estatus:=0;
...
var swsalir:boolean;
    ss,rsp,xestatus:string;
    xcmnd,xpc,xpos,xcomb,xmang,ximp,i,xfolio:integer;
    ...
    with DMCONS do begin
      ...
      if not Q_ConsIb.Eof then begin
        ...
        if (xpos in [1..MaxPosiciones]) then with TPosCarga[xpos] do begin
          xpc:=Q_ConsIbPosicion.AsInteger;
          if xpc in [1..3] then
            xmang:=PosManguera[xpc]
          else xmang:=1;
          with TMangueras[xmang] do begin
            if (estatus=1) then begin
              TipoPago:=Q_ConsIbTipoPago.asinteger;
              ximp:=trunc(Q_ConsIbValor.AsFloat*100+0.5);
              if Q_ConsIbMonVol.AsString='V' then begin // litros
                TMangueras[xmang].SwPresetImp:=false;
                TMangueras[xmang].impopreset:=Q_ConsIbValor.AsFloat;
                ComandoConsola('L'+inttoclavenum(xmang,2)+InttoClaveNum(ximp,6));
              end
              else begin
                TMangueras[xmang].SwPresetImp:=true;
                TMangueras[xmang].impopreset:=Q_ConsIbValor.AsFloat;
                ComandoConsola('S'+inttoclavenum(xmang,2)+InttoClaveNum(ximp,6));
              end;
            end
            else
              rsp:='Posición de Carga no Disponible';
      ...
      for xcmnd:=1 to 40 do if (TabCmnd[xcmnd].SwActivo)and(not TabCmnd[xcmnd].SwResp) then begin
              ...
              for xpos:=1 to MaxPosiciones do begin
                for i:=1 to MaxMangueras do begin
                  if (TMangueras[i].PosCarga=xpos) then begin
                    ComandoConsola('C'+inttoclavenum(i,2));
                    esperamiliseg(100);
          ...
          if (xpos in [0..MaxPosiciones]) then begin
            if xpos=0 then begin
              for xpos:=1 to MaxPosiciones do begin
                for i:=1 to MaxMangueras do begin
                  if (TMangueras[i].PosCarga=xpos) then begin
                    ComandoConsola('D'+inttoclavenum(i,2));
                    esperamiliseg(100);
          ...
          if (xmang in [1..MaxMangueras]) then begin
            ...
            if rsp='OK' then begin
              ...
              if TMangueras[xmang].SwPrepagoM then begin
                ComandoConsola('D'+inttoclavenum(xmang,2));
                esperamiliseg(100);
              end;
        ...
        // ORDENA CARGA DE COMBUSTIBLE (LITROS)
          ...
          if (xmang in [1..MaxMangueras]) then begin
            try
              ...
              if (xLitros<1)or(xLitros>999) then
                rsp:='Litros fuera de rango válido: de 1.00 a 999.00';
            except
              xLitros:=0;
              rsp:='Error en Valor';
            end;
        ...
        // CMND: ACTIVA FLUJO MINIMO
          ...
          if true then begin //(Licencia2Ok)or(Licencia3Ok) then begin
            ...
            ActualizaAdic(0);
            for xmang:=1 to MaxMangueras do begin
              ComandoConsola('W'+IntToClaveNum(xMang,2)+IntToClaveNum(0,2));
              esperamiliseg(400);
              TMangueras[xmang].SwAdic:=true;
            end;
          end
  ...
  try
    with DMCONS do begin
      ...
      if (ContadorTot>=100) then begin
        ContadorTot:=0;
        inc(ContadorTotPos);
        if ContadorTotPos in [1..MaxMangueras] then begin
          TMangueras[ContadorTotPos].SwCargaTotales:=true;
        end
