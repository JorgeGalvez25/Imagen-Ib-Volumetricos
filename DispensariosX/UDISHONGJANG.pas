...
var
  ...
  TValor:array[1..32,1..3] of integer;
    ...
    try
      ...
      while not Q_Pcar.Eof do begin
        ...
        if (xpos<=DMCONS.MaximoDePosiciones) then begin
          TValor[xpos,1]:=Trunc(10*Q_PcarSlowFlow.AsFloat+0.01);
          TValor[xpos,2]:=Trunc(10*Q_PcarSlowFlow2.AsFloat+0.01);
          TValor[xpos,3]:=Trunc(10*Q_PcarSlowFlow3.AsFloat+0.01);
        end;
        Q_Pcar.Next;
      end;
      Q_Pcar.Active:=true;
  ...
  try
    ...
    if not SwEsperaCmnd then begin
      ...
      Case CmndProc of
        ...
        // REVISA COMANDOS
        'W':begin
              if (swcmndw)and(xtryw<=1) then begin
                inc(xtryw);
                if xtryw=1 then begin
                  esperamiliseg(400);
                  Ap1.Open:=false;
                  Ap1.Open:=true;
                  esperamiliseg(400);
                  ComandoConsola('W'+IntToClaveNum(xmangw,2)+IntToClaveNum(xvalorw,2));
                end;
              end
              else if ApCola>0 then begin
                Swcmndw:=false;xtryw:=0;
                SacaDeCola(xcmnd);
  ...
  try
    if not SwInicCierre then with DMCONS do begin
      ...
      for xcmnd:=1 to 40 do if (TabCmnd[xcmnd].SwActivo)and(not TabCmnd[xcmnd].SwResp) then begin
          ...
          if (xmang in [1..MaxMangueras]) then begin
            if (TMangueras[xmang].estatus in [1,3])then begin
              ...
              if rsp='OK' then begin
                ...
                TMangueras[xmang].swflujovehiculo:=false;
                if DMCONS.FlujoPorVehiculo='Si' then begin
                  ss:=ExtraeElemStrSep(TabCmnd[xcmnd].Comando,8,' ');
                  if ss<>'' then begin
                    TMangueras[xmang].swflujovehiculo:=true;
                    TMangueras[xmang].flujovehiculo:=StrToIntDef(ss,0);
                    MeteACola('W'+IntToClaveNum(xmang,2)+IntToClaveNum(TMangueras[xmang].flujovehiculo,2))
                  end;
                end;
        ...
        else if ss='FLUCERO' then begin // FLUJO MINIMO
          if true then begin //(Licencia2Ok)or(Licencia3Ok) then begin
            rsp:='OK';
            ActualizaAdic(0);
            for xmang:=1 to MaxMangueras do begin
              if (TMangueras[xmang].estatus in [5,8]) then begin
                TMangueras[xmang].SwPendW:=true;
              end
              else begin
